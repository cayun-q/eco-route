...
export function greatCirclePolyline(a: LatLng, b: LatLng, minPoints = 48): [number, number][] {
  const dist = haversineKm(a, b);
  const steps = Math.max(minPoints, Math.ceil(dist / 12));
  const points: [number, number][] = [];
  for (let i = 0; i <= steps; i += 1) points.push(interpolateGreatCircle(a, b, i / steps));
  return points;
}
...
export function mockRoute(a: LatLng, b: LatLng, mode: TransportMode): { distanceKm: number; durationMin: number; polyline: [number, number][] } {
  const distanceKm = round3(haversineKm(a, b));
  return { distanceKm, durationMin: durationMinForMode(distanceKm, mode), polyline: greatCirclePolyline(a, b) };
}
...
export function decodePolyline(encoded: string): [number, number][] {
  ...
  while (index < encoded.length) {
    ...
    do { b = encoded.charCodeAt(index) - 63; index += 1; result |= (b & 0x1f) << shift; shift += 5; } while (b >= 0x20);
