...
export async function loadFactors(): Promise<FactorRow[]> {
  const { rows } = await pool.query<FactorRow>(
    "SELECT mode, g_per_km, source, notes, updated_at FROM emission_factors ORDER BY mode",
  );
  return rows;
}
...
export async function loadFactor(mode: string): Promise<FactorRow | null> {
  const { rows } = await pool.query<FactorRow>(
    "SELECT mode, g_per_km, source, notes, updated_at FROM emission_factors WHERE mode = $1",
    [mode],
  );
  return rows[0] ?? null;
}
