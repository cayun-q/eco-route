...
import type { LogMethod } from "@carbonroute/shared";
...
import type { DisplayPrecision, MeasurementSystem } from "../format";
    ...
    <Screen>
      <ScrollView contentContainerStyle={styles.wrap}>
        ...
        <PreferenceSection label="Trip defaults">
          <Option title="Automatic logging" detail="Luma builds the itinerary" selected={defaultLoggingMethod === "automatic"} onPress={() => void setDefaultLoggingMethod("automatic" satisfies LogMethod)} styles={styles} />
          <Option title="Manual itinerary" detail="Start new trips in manual mode" selected={defaultLoggingMethod === "manual"} onPress={() => void setDefaultLoggingMethod("manual" satisfies LogMethod)} styles={styles} />
        </PreferenceSection>
        ...
        <PreferenceSection label="Display precision">
          <Option title="Simple" detail="Cleaner rounded values" selected={displayPrecision === "simple"} onPress={() => void setDisplayPrecision("simple" satisfies DisplayPrecision)} styles={styles} />
          <Option title="Detailed" detail="Show extra decimal precision" selected={displayPrecision === "detailed"} onPress={() => void setDisplayPrecision("detailed" satisfies DisplayPrecision)} styles={styles} />
        </PreferenceSection>
        ...
        <Text style={styles.note}>
          These preferences are saved on this device. Unit and precision changes only affect display; stored route and emissions data stays unchanged.
        </Text>
      </ScrollView>
    </Screen>
  );
...
function PreferenceSection({ label, children }: { label: string; children: React.ReactNode }) {
...
function makeStyles(colors: ThemeColors) {
  return StyleSheet.create({
    ...
    optionTitle: { fontFamily: font.bodyMed, fontSize: 16, color: colors.ink },
    optionDetail: { fontFamily: font.body, fontSize: 13, color: colors.muted },
